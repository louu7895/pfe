...
32 & Réinitialiser mot de passe     & Employé + Responsable RH + Administrateur     & réinitialiser mon mot de passe    & récupérer l'accès à mon compte   & 4 & 2 \\ \hline
...
\subsubsection*{Description textuelle : cas d'utilisation « Réinitialiser le mot de passe »}
Ce tableau décrit en détail le scénario nominal et les cas d'exception du cas d'utilisation « Réinitialiser le mot de passe », incluant les préconditions, les étapes d'interaction entre l'utilisateur et le système, ainsi que la post-condition attendue.

\begin{longtable}{|p{5cm}|p{11cm}|}
\hline
\textbf{Nom du cas d'utilisation} & \textbf{Réinitialiser le mot de passe} \\ \hline
\textbf{Acteurs}                  & Utilisateur \\ \hline
\textbf{Objectif}                 & Permettre à l'utilisateur de réinitialiser son mot de passe en cas d'oubli \\ \hline
\textbf{Préconditions}            & L'utilisateur possède un compte associé à une adresse e-mail valide \\ \hline
\textbf{Scénario nominal} &
1. L'utilisateur accède à la page de connexion. \newline
2. Le système affiche le formulaire de connexion. \newline
3. L'utilisateur saisit ses coordonnées de connexion. \newline
4. Le système détecte que les informations sont incorrectes. \newline
5. Le système affiche un message d'erreur. \newline
6. L'utilisateur clique sur « Mot de passe oublié ». \newline
7. Le système affiche le formulaire de réinitialisation. \newline
8. L'utilisateur saisit son adresse e-mail. \newline
9. Le système vérifie l'existence et la validité de l'adresse e-mail. \newline
10. Le système génère un nouveau mot de passe. \newline
11. Le système envoie le nouveau mot de passe à l'adresse e-mail de l'utilisateur. \newline
12. L'utilisateur reçoit le nouveau mot de passe avec succès. \\ \hline
\textbf{Scénario d'exception} &
1.1 Si l'adresse e-mail n'existe pas dans la base de données : \newline
Le système affiche un message d'erreur. \newline
1.2 Si l'adresse e-mail n'est pas vérifiée : \newline
Le système affiche un message d'erreur. \\ \hline
\textbf{Post-condition} & L'utilisateur reçoit un nouveau mot de passe dans sa boîte e-mail \\ \hline
\caption{Description textuelle du cas d'utilisation « Réinitialiser le mot de passe »}
\end{longtable}
...
2. Le système propose plusieurs méthodes d'inscription (formulaire, Google, etc.). \newline
3. L'utilisateur choisit une méthode d'inscription. \newline
4. L'utilisateur saisit les informations demandées ou autorise l'accès via le service externe. \newline
5. Le système vérifie les informations reçues. \newline
6. Le système enregistre les informations dans la base de données. \newline
7. L'utilisateur est redirigé vers son tableau de bord. \\ \hline
\textbf{Scénario d'exception} &
...
Ce diagramme de séquence représente les échanges entre l'utilisateur, l'interface et le système lors du processus d'authentification, depuis la saisie des identifiants jusqu'à l'affichage du tableau de bord ou l'affichage d'un message d'erreur en cas d'identifiants invalides.

\subsubsection*{Diagramme de séquence du cas d'utilisation « Réinitialiser le mot de passe »}

Ce diagramme décrit le processus de récupération du compte. L'utilisateur fait une demande de réinitialisation, puis le système vérifie l'adresse e-mail avant d'envoyer un nouveau mot de passe sécurisé par courriel.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{projet/images/img sequence s1/m.png}
    \caption{Diagramme de séquence du cas d'utilisation « Réinitialiser le mot de passe »}
    \label{fig:interface6}
\end{figure}
...
Ce diagramme présente les échanges entre le Responsable RH et le système lors de l'inscription, en montrant la vérification des données saisies, l'enregistrement dans la base de données et la redirection vers le tableau de bord.
...
Ce diagramme illustre les interactions entre le Responsable RH et le système lors de la création d'un compte employé, incluant la validation des données saisies, l'enregistrement du compte et l'envoi automatique du lien d'activation par e-mail.
...
Ce diagramme représente le processus de suppression d'un compte employé, en mettant en évidence l'étape de demande de confirmation par le système avant de procéder à la suppression définitive des données associées.
...
Ce diagramme décrit les échanges entre l'Administrateur et le système lors de la modification d'un compte Responsable RH, depuis la sélection du compte et la mise à jour des champs souhaités jusqu'à l'enregistrement des modifications par le système.
...
Ce diagramme de classe présente la structure statique du module d'authentification et de gestion des comptes, avec les attributs et méthodes des classes Administrateur, Responsable RH et Employé, ainsi que leurs relations d'héritage et d'association.
...
Cette figure présente la page de connexion de la plateforme, permettant à chaque utilisateur de saisir ses identifiants afin d'accéder à son espace personnel selon son rôle.
...
Cette figure illustre le formulaire d'inscription destiné au Responsable RH, présentant les champs nécessaires à la création d'un nouveau compte sur la plateforme.

\subsubsection*{Interface Réinitialiser mot de passe}

Cette interface permet à l'utilisateur de récupérer l'accès à son compte en saisissant son adresse e-mail afin de recevoir un nouveau mot de passe par e-mail.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{projet/images/interface/s1/motdepasee oblie.jpeg}
    \caption{Interface Réinitialiser mot de passe}
    \label{fig:interface3}
\end{figure}

Cette figure montre la page de réinitialisation du mot de passe, permettant à l'utilisateur de saisir son adresse e-mail pour recevoir un nouveau mot de passe et récupérer l'accès à son compte.
...
Cette figure présente l'interface de gestion des comptes employés, offrant au Responsable RH un tableau de bord centralisé pour créer, modifier et supprimer les comptes des employés.
...
Cette figure illustre l'interface d'administration dédiée à la gestion des comptes Responsables RH, permettant à l'Administrateur d'effectuer les opérations de création, modification et suppression de ces comptes.
...
3. Le système affiche ses absences et retards. \\ \hline
\textbf{Scénario d'exception} &
...
Ce diagramme illustre les interactions entre le Responsable RH, la plateforme et les caméras lors de la configuration, montrant les étapes de détection, de paramétrage des caméras d'entrée et de sortie, et d'activation du module de pointage automatique.
...
Ce diagramme représente le flux d'enregistrement biométrique, depuis la capture du visage par la caméra jusqu'au traitement par le module d'intelligence artificielle et le stockage des données faciales dans la base de données.
...
Ce diagramme présente les échanges entre l'employé et le système lors de la mise à jour de ses informations personnelles, incluant la vérification de la validité des données saisies et la confirmation de l'enregistrement des modifications.
...
Ce diagramme illustre la consultation de l'historique des absences et retards par l'employé, montrant la requête envoyée vers la base de données et l'affichage des résultats dans la section dédiée du tableau de bord.
...
Ce diagramme représente le processus complet de pointage facial, depuis l'activation de la caméra lors du check-in jusqu'à la vérification de l'identité par reconnaissance faciale et l'enregistrement des heures d'entrée et de sortie dans le système.

\subsubsection{Diagramme de classe du sprint 2}

Ce diagramme de classe représente les composants relatifs au pointage intelligent et au suivi de présence, notamment la configuration des caméras, la reconnaissance faciale, les présences, les absences et les informations personnelles des employés.
...
Cette figure présente l'interface de configuration des caméras, permettant au Responsable RH de détecter les caméras disponibles et de définir les caméras d'entrée et de sortie pour activer le pointage automatique.
...
Cette figure illustre l'interface d'enregistrement du visage, activée lors de la première connexion de l'employé, permettant la capture et le stockage des données biométriques faciales nécessaires au pointage automatique.
...
Cette figure présente la page de profil de l'employé, offrant la possibilité de consulter et de modifier ses informations personnelles telles que le nom, l'adresse e-mail et le mot de passe.
...
Cette figure illustre l'interface de suivi des absences, affichant à l'employé l'historique détaillé de ses absences et retards enregistrés par le système de pointage.
...
Cette figure présente l'interface de pointage facial, permettant à l'employé en télétravail d'enregistrer ses heures d'arrivée et de départ via les boutons « Check In » et « Check Out », en utilisant la reconnaissance faciale pour valider son identité.
